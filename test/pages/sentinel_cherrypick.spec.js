...
describe('sentinel cherrypick', () => {
  ...
  describe('#getPlates', () => {
    it('calls getPlatesFromBoxBarcodes', async () => {
      labwhere.getPlatesFromBoxBarcodes = vi.fn()
      wrapper.vm.handleGetPlatesResponse = vi.fn()
  ...
  describe('batch creation, all selected', () => {
    ...
    describe('#createBatch', () => {
      it('calls createCherrypickBatch', async () => {
        sequencescape.createCherrypickBatch = vi.fn()
        wrapper.vm.handleCreateBatchResponse = vi.fn()
        await wrapper.vm.createBatch()
        expect(sequencescape.createCherrypickBatch).toHaveBeenCalled()
      })
    })

    describe('#handleCreateBatchResponse', async () => {
      let response

      it('on success it shows a link', async () => {
        response = {
          success: true,
          data: {
            id: '4',
